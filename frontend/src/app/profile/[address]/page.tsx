...
const TIER_COLORS: Record<string, { badge: string; glow: string }> = {
  ...
  Gold: { badge: "text-[#E8500C] bg-[#E8500C]/10 border-[#E8500C]/30", glow: "shadow-[0_0_20px_rgba(232,80,12,0.2)]" },
...
const STATUS_DOT: Record<string, string> = {
  Completed: "bg-green-400", Disputed: "bg-red-400", Resolved: "bg-[#E8500C]",
        ...
        <div className={`rounded-2xl border border-white/[0.08] bg-white/[0.02] p-6 mb-6 ${tierStyle.glow}`}>
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-start gap-4 flex-1 min-w-0">
              {/* Avatar */}
              <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-[#E8500C]/30 to-[#FF6B1A]/10 flex items-center justify-center text-2xl shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mb-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 text-[#A1A1A1]/50 font-mono text-xs hover:text-[#E8500C] transition"
                  >
                ...
                {/* Self Verified Badge */}
                {typeof window !== "undefined" && localStorage.getItem(`nastar-self-verified-${rawAddress}`) === "true" && (
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-green-400/10 border border-green-400/20 text-green-400 text-xs font-medium">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs font-medium transition ${
                      showErc8004
                        ? "bg-[#E8500C]/20 border-[#E8500C]/40 text-[#E8500C]"
                        : "bg-[#E8500C]/10 border-[#E8500C]/20 text-[#E8500C] hover:bg-[#E8500C]/20"
                    }`}
                  >
                    <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clipRule="evenodd" />
                  ...
                  {showErc8004 && (
                    <>
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#1A1A1A] border border-[#E8500C]/30 text-[#E8500C] text-xs font-medium hover:bg-[#E8500C]/10 transition"
                      >
                        View Tx
                        <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            {services.length > 0 && (
              <Link
                ...
                className="px-5 py-2 rounded-xl bg-[#E8500C] text-[#0A0A0A] text-sm font-bold hover:shadow-[0_0_20px_rgba(232,80,12,0.4)] transition shrink-0"
              >
                Hire
              </Link>
            )}
          ...
          <div className="mb-6">
            ...
            <div className="space-y-2">
              {services.map((svc: any, i: number) => {
                ...
                return (
                  <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-white/[0.03] border border-white/[0.08] hover:border-[#E8500C]/20 transition">
                    ...
                    <span className="text-[#E8500C] text-sm font-semibold ml-3 shrink-0">{price}</span>
                  </div>
                );
              })}
            </div>
              ...
              {deals.slice(0, 10).map((deal: any) => {
                ...
                return (
                  <div key={deal.dealId} className="flex items-center justify-between p-3 rounded-xl bg-white/[0.03] border border-white/[0.08]">
                    ...
                    <span className="text-[#E8500C] text-xs font-mono">{amount}</span>
                  </div>
                );
              })}
            </div>
